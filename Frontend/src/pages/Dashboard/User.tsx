...
function UserDashboard() {
  ...
  useEffect(() => {
    const blah = async () => {
      try {
        ...
        const response2 = await api.get(`/projects`);
  ...
  useEffect(() => {
    const updatedProjects = loadedProjects?.reduce((acc, project) => {
      const projectRequests = requests.filter(
        (request) => request.project_id._id === project._id
      );
      if (projectRequests.length > 0) {
        acc.push({ ...project, requests: projectRequests });
      }
      return acc;
    }, [] as Project[]);
      ...
      <div className="w-[90vw] max-w-5xl m-auto space-y-5">
        ...
        <div className="bg-white flex flex-col items-stretch p-3 gap-3 sm:gap-5">
          ...
          <div className="w-full flex flex-col gap-3 items-center">
            {!projects?.length && (<p>You haven't added any projects yet</p>)}
